...
def sign_up():
    if request.method=='POST':
        ...
        if user:
            ...
            flash("Password must be larger than 6 characters",category="error")
        else:
            new_user=User(email=email,first_name=first_name,last_name=last_name,password=generate_password_hash(password1,"sha256"))
